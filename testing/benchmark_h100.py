...
def bench_fig11():
    ...
    for b, s in [(B, S) for B in Batches for S in seqlens]:
        result_dict = bench_attention(
            "causal_softmax_attn", b, 16, 1, s, 192, 128, require_grad=False
        )
    ...
    for b, s in [
        ...
        result_dict = bench_attention(
            "mla_attn", b, 128, 1, s, 576, 512, head_k=1, head_v=1
        )
    ...
    for b, s in [
        ...
        result_dict = bench_attention(
            "sparse_gqa", b, 32, 1, s, 128, 128, head_k=8, head_v=8
        )
...
def bench_attention(
    ...
    if attn_type == "causal_softmax_attn":
        result_dict = bench_softmaxattention(
            Batch, head, seqlen_q, seqlen_kv, dim_qk, dim_v, require_grad=require_grad
        )
        ...
        result_dict = bench_mamba2_ssm(
            Batch, head, seqlen_q, dim_qk, dim_v, HK=head_k, HV=head_v
        )
    elif attn_type == "mla_attn":
        result_dict = bench_mla_decode(
            Batch, head, seqlen_kv, dim_qk, dim_v, HKV=head_k
        )
    elif attn_type == "sparse_gqa":
        result_dict = bench_sparse_gqa_decode(
            Batch, head, head_k, seqlen_kv, dim_qk, dim_v
        )
    else:
...
def bench_softmaxattention(
    B, H, Sq, S, D, DV, device="cuda", dtype=torch.float16, require_grad=True
):
...
def bench_gated_retention(
    B, H, S, D, DV, device="cuda", dtype=torch.bfloat16, require_grad=True
):
    ...
    try:
        ...
        fwd_lat_ref2 = do_bench(
            lambda: torch_simple_gla(q1, k1, v1, g1, chunk_size=512)
        )
...
def bench_retnet_recurrent(
    ...
    try:
        ...
        fwd_lat_ref = do_bench(lambda: chunk_retention(q1, k1, v1, head_first=True)[0])
        if require_grad:
...
def bench_retention_parallel(
    B, H, S, D, DV, device="cuda", dtype=torch.float16, require_grad=False
):
...
if __name__ == "__main__":
    ...
    log_success("Figure 11 plotted and saved to figure11_h100.pdf")
