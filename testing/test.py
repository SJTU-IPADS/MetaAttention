...
def run_test_with_info(func, *args, **kwargs):
    ...
    try:
        ...
        # green
        print(
            colored(f"✔ [PASS] {func_name}", "green", attrs=["bold"])
...
def check_mamba2(
    ...
    out_ref = ssd_chunk_scan_combined_ref(
        value, dt_mamba, A_mamba, key, query, chunk_size=64
    ).to(dtype)
    if require_grad:
...
def check_sigmoid_attention(
    ...
    softmax_bias = torch.tensor([1], device=device, dtype=accum_dtype).uniform_(
        -10.0, 2.0
    )
...
def check_relu_attention(
    ...
    if require_grad:
        ...
        o.backward(do, retain_graph=True)
        ref_o.backward(do, retain_graph=True)
        assert_close(query.grad, query1.grad, rtol=1e-1, atol=1e-1, mismatch_ratio=1e-3)
        assert_close(key.grad, key1.grad, rtol=1e-1, atol=1e-1, mismatch_ratio=1e-3)
