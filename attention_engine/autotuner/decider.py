...
def decider(qkv_meta, hardware_meta) -> Tuple[bool, dict]:
    ...
    block_K = [
        bk
        for bk in range(block_K_stride, block_K_max + 1, block_K_stride)
        if dim_qk % bk == 0
    ]
    ...
    num_threads = [
        nt for nt in range(num_threads_stride, num_threads_max + 1, num_threads_stride)
    ]
    ...
    for bm, bn, bk, qk_mem, acco_mem, nt, stage in config_iter:
        shared_mem, reg_num = memory_usage(
            bm, bn, bk, dim_v, qk_mem, acco_mem, nt, stage, dtype
        )
        if (
            shared_mem <= hardware_meta.smem_cap
            and reg_num * nt <= hardware_meta.reg_cap
            and reg_num <= hardware_meta.register_per_thread
        ):
            # register fuse, warp row
            if bm % (
                (nt / hardware_meta.threads_per_mma) * hardware_meta.mma_primitive[0]
            ):
                continue
