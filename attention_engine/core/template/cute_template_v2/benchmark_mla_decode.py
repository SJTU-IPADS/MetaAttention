...
for attn_variant in attn_variants[3:5]:
    nheads_kv = (
        ...
        if attn_variant == "mha"
            ...
            max(nheads_q // 8, 1)
            if attn_variant == "gqa"
            else (1 if attn_variant == "mla" else 2)
        )
    )
    headdim = 64 if attn_variant in ["mla", "gla"] else 128
    headdim_v = (
        512 if attn_variant == "mla" else (256 if attn_variant == "gla" else headdim)
    )
    ...
    should_run_flashmla = (
        attn_variant == "mla" and page_size == 64 and flash_mla_with_kvcache is not None
    )
    ...
    for seqlen in [s * 1024 for s in [1, 2, 4, 8, 16, 32, 64]]:
        ...
        cache_seqlens = torch.tensor(
            [seqlen] * batch_size, device=device, dtype=torch.int
        )
        num_splits = 0
        q = torch.randn(
            batch_size, seqlen_q, nheads_q, headdim, dtype=dtype, device=device
        )
        try:
            v_cache = torch.randn(
        ...
        qv = (
            torch.randn(
                batch_size, seqlen_q, nheads_q, headdim_v, dtype=dtype, device=device
            )
            if has_qv
            else None
        )
        ...
        if should_run_flashmla:
            ...
            mla_metadata = get_mla_metadata(
                cache_seqlens, seqlen_q * nheads_q // nheads_kv, nheads_kv
            )
        ...
        print(
            f"Seqlen = {seqlen}, FA3 time{'' if not use_bench_cudagraph else ' w CUDA Graph'}: {t0 * 1e3:.1f} us, {mem_io * 1e-9 / (t0 * 1e-3):.0f} GB/s, {flops * 1e-12 / (t0 * 1e-3):.0f} TFLOPS/s"
        )
        if should_run_flashmla:
            print(
                f"Seqlen = {seqlen}, FlashMLA time{'' if not use_bench_cudagraph else ' w CUDA Graph'}: {t1 * 1e3:.1f} us, {mem_io * 1e-9 / (t1 * 1e-3):.0f} GB/s, {flops * 1e-12 / (t1 * 1e-3):.0f} TFLOPS/s"
            )
