...
def get_configs():
    ...
    _configs = list(
        itertools.product(num_splits, block_Ns, block_Ms, stages, shared_fuse)
    )
    configs = [
        {
            "block_N": c[1],
...
def flashattn(batch, heads, kv_head_num, seqlen_kv, dim, pe_dim, tune=False):
    ...
    def kernel_func(block_N, block_H, num_split, num_stages, shared_fuse):
        ...
        def flash_attn(
            ...
            with T.Kernel(batch, heads // min(block_H, kv_group_num), threads=256) as (
                bx,
                by,
            ):
                ...
                if shared_fuse:
                    T.copy(
                        Q[bx, 0, by * VALID_BLOCK_H : (by + 1) * VALID_BLOCK_H, :],
                        Q_shared,
                    )
                    T.copy(
                        Q_pe[bx, 0, by * VALID_BLOCK_H : (by + 1) * VALID_BLOCK_H, :],
                        Q_pe_shared,
                    )
                else:
                    T.copy(
                ...
                for k in T.Pipelined(loop_range, num_stages=num_stages):
                    T.copy(
                        KV[bx, k * block_N : (k + 1) * block_N, cur_kv_head, :],
                        KV_shared,
                    )
                    T.copy(
                    ...
                    if shared_fuse:
                        T.gemm(
                            S_shared, KV_shared, acc_o, policy=T.GemmWarpPolicy.FullCol
                        )
                    else:
                        T.gemm(
                            acc_s_cast,
                ...
                if shared_fuse:
                    ...
                    T.copy(
                        O_shared,
                        Output[bx, 0, by * VALID_BLOCK_H : (by + 1) * VALID_BLOCK_H, :],
                    )
                else:
                    T.copy(
                        acc_o,
        ...
        def flash_attn_split(
            ...
            with T.Kernel(
                batch, heads // min(block_H, kv_group_num), num_split, threads=256
            ) as (bx, by, bz):
                Q_shared = T.alloc_shared([block_H, dim], dtype)
                Q_local = T.alloc_fragment([block_H, dim], dtype)
                S_shared = T.alloc_shared([block_H, block_N], dtype)
                ...
                T.copy(
                    logsum,
                    glse[bx, by * VALID_BLOCK_H : (by + 1) * VALID_BLOCK_H, bz, 0],
                )
                if shared_fuse:
                    T.copy(acc_o, O_shared)
                    T.copy(
                        O_shared,
                        Output_partial[
                            bx, 0, by * VALID_BLOCK_H : (by + 1) * VALID_BLOCK_H, bz, :
                        ],
                    )
                else:
                    T.copy(
...
def tune(tune_file, kernel_profiler, problem_keys) -> Tuple:
    ...
    if tuned_config is None:
        ...
        result = kernel_profiler(**problem_keys)
...
if TUNE:
    ...
    tuned_config, tuned_latency = tune(
        f"tuned_config/{attn_device.name}/mla_decode.json",
        partial(flashattn, tune=True),
        problem_keys,
    )
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, q_pe, kv, k_pe):
        global num_split
        glse = torch.empty(
            (q.shape[0], q.shape[2], num_split, q.shape[1]),
            dtype=q.dtype,
            device=q.device,
        )
        Output_partial = torch.empty(
            (q.shape[0], q.shape[1], q.shape[2], num_split, kv.shape[-1]),
            dtype=q.dtype,
            device=q.device,
        )
