...
shape_idx_onchip_dim_map = ["seq_len", "seq_len_kv"]
...
class KernelOptionsBase:
    ...
    def add_output_tensor(
        ...
        dtype: str,
        global_name=None,
        global_idx: List[sp.Symbol] = None,
        global_dim_map: List[int] = None,
    ):
        if global_name is None:
            global_name = f"g_{tensor_name}"
        new_tensor = SymbolScalar(
            tensor_name, Var(tensor_name), shape_idx=tile_shape, dtype=dtype
        )
        if is_sharedmem:
            ...
            if tensor_name in self.fragment_tensors.keys():
                logging.warning(
                    f"Tensor {tensor_name} already exists in fragment memory"
                )
            self.fragment_tensors[tensor_name] = new_tensor
        if global_idx is not None:
            self.global_tensors_output[global_name] = SymbolScalar(
                global_name, Var(global_name), shape_idx=global_shape, dtype=dtype
            )
    ...
    def add_input_tensor(
        ...
        if is_sharedmem:
            ...
            self.fragment_tensors[tensor_name] = new_tensor
        if global_idx is not None:
            self.global_tensors_input[f"g_{tensor_name}"] = SymbolScalar(
                f"g_{tensor_name}",
                Var(f"g_{tensor_name}"),
                shape_idx=global_shape,
                dtype=dtype,
            )
            self.copy_maps.append(
                CopyMap(
    ...
    def add_intermediate_tensor(
        self, tensor_name: str, shape, is_sharedmem: bool, dtype: str
    ):
        tensor = SymbolScalar(
            tensor_name, Var(tensor_name), shape_idx=shape, dtype=dtype
        )
        if is_sharedmem:
            if tensor_name in self.shared_tensors.keys():
                logging.warning(f"Tensor {tensor_name} already exists in shared memory")
...
class lowerOnlineFuncOutput:
    def __init__(
        ...
        online_func_def,
        call_online_func,
        online_func_epilogue,
        online_rowscales_update,
        isused_doosum,
        final_rowscales_length,
...
def lower_kernel(
    kernel_options: KernelOptionsBase, kernel_template: lowerKernelBaseOutput
):
    # generate input args
        ...
        if copy_map.dst.name in kernel_options.global_tensors_output.keys():
            output_args_copy_epilogue_code.add_line(
                store_op(
                    copy_map.src,
                    copy_map.dst,
                    copy_map.idx_dim_map,
                    dst_dim_list=list(range(len(copy_map.idx_list))),
                    dst_idx_list=copy_map.idx_list,
                )
            )
        ...
        if copy_map.src.name in kernel_options.global_tensors_input.keys():
            input_args_copy_prologue_code.add_line(
                load_op(
                    ...
                    copy_map.dst,
                    copy_map.idx_dim_map,
                    src_dim_list=list(range(len(copy_map.idx_list))),
                    src_idx_list=copy_map.idx_list,
                )
            )
...
def lower_online_func(
    ...
    scores = SymbolicArray(
        ...
        shape_idx=[kernel_options.tile_M, kernel_options.tile_N],
    )
    ...
    acco = SymbolicArray(
        lower_output.acc_o,
        Var(lower_output.acc_o),
        shape_idx=[kernel_options.tile_M, kernel_options.dimv],
    )
    ...
    for k, v in new_online_rowscales.items():
        if v.varname == k:
            continue
        online_rowscales_update.add_line(copy_op(v, online_rowscales[k]))

    # 5. final_rowscales block
    for k, v in online_rowscales.items():
        online_rowscales[k].clear_codegen()
    acco_new, new_final_rowscales = online_func.online_fwd_epilogue(
        acco, online_rowscales, b, h, q_idx
    )
    ...
    if bwd_kernel_options is not None:
        qkT = SymbolScalar(
            lower_output.qkT,
            Var(lower_output.qkT),
            shape_idx=[str(bwd_kernel_options.tile_M), str(bwd_kernel_options.tile_N)],
        )
        kv_idx = SymbolScalar("kv_idx", Var("kv_idx"))
        dsT = SymbolScalar(
            lower_output.dsT,
            Var(lower_output.dsT),
            shape_idx=[str(bwd_kernel_options.tile_M), str(bwd_kernel_options.tile_N)],
        )
        doosum_shared = SymbolScalar(
            lower_output.doosum_shared,
            Var(lower_output.doosum_shared),
            shape_idx=["1", str(bwd_kernel_options.tile_N)],
        )

        for k, v in online_func.final_rowscales.items():
            final_rowscales_bwd[k] = SymbolScalar(
                f"{k}_shared", Var(f"{k}"), shape_idx=["1", "block_N"]
            )
        ...
        custom_bwd_inputs_init = (
            "doosum_shared = T.alloc_shared([1, block_N], accum_dtype, scope='shared')"
            if isused_doosum
            else ""
        )
        ...
        for k, v in final_rowscales_bwd.items():
            final_rowscales_load += (
                f"T.copy(g_{k}[bz, bx, k * block_N : (k + 1) * block_N], {v.varname})\n"
            )
        custom_bwd_inputs_load = (
...
def lower_score_mod(
    ...
    custom_fwd_inputs,
    lower_output: lowerOutput,
    kernel_options: AttnFwdKernelOption,
    bwd_kernel_options: AttnBwdKernelOption,
):
    # 1. init input vars
    scores = SymbolScalar(
        lower_output.scores,
        Var(lower_output.scores),
        shape_idx=[str(kernel_options.tile_M), str(kernel_options.tile_N)],
    )
    ...
    if bwd_kernel_options is not None:
        ...
        qkT = SymbolScalar(
            lower_output.qkT,
            Var(lower_output.qkT),
            shape_idx=[bwd_kernel_options.tile_M, bwd_kernel_options.tile_N],
        )
        dsT = SymbolScalar(
            lower_output.dsT,
            Var(lower_output.dsT),
...
def lower_custom_inputs(
    custom_fwd_inputs, lower_output: lowerOutput, kernel_options: KernelOptionsBase
):
    ...
    for k, v in custom_fwd_inputs.input_tensors.items():
        ...
        shape_idx_copy_sp = [
            (
                shape_idx_map_sp[shape]
                if shape in shape_idx_map_sp.keys()
                else sp.simplify("0")
            )
            for shape in v.shape_idx
        ]
        shape_idx_block = [
            (
                shape_idx_onchip_map[shape]
                if shape in shape_idx_onchip_map.keys()
                else shape
            )
            for shape in v.shape_idx
        ]
        ...
        [
            (
                shape_idx_onchip_step_map_sp[shape]
                if shape in shape_idx_onchip_step_map_sp.keys()
                else sp.simplify(shape)
            )
            for shape in v.shape_idx
        ]
        shape_idx_dim_map = [
            idx
            for idx, shape in enumerate(v.shape_idx)
            if shape in shape_idx_onchip_dim_map
        ]
        custom_input_dtype = "accum_dtype"

        kernel_options.fragment_tensors[k] = SymbolScalar(
            k, Var(k), shape_idx=shape_idx_block, dtype=custom_input_dtype
        )
        kernel_options.global_tensors_input[f"g_{k}"] = SymbolScalar(
            f"g_{k}", Var(f"g_{k}"), shape_idx=v.shape_idx, dtype=custom_input_dtype
        )

        # tl copy bug when "1"
        if RECURRENT_DIM not in shape_idx_block:
            kernel_options.copy_maps.append(
                CopyMap(
                    kernel_options.global_tensors_input[f"g_{k}"],
                    kernel_options.fragment_tensors[k],
                    shape_idx_copy_sp,
                    shape_idx_dim_map,
                )
            )
            ...
            kernel_options.shared_tensors[f"{k}_shared"] = SymbolScalar(
                f"{k}_shared",
                Var(f"{k}_shared"),
                shape_idx=shape_idx_block,
                dtype=custom_input_dtype,
            )
            custom_fwd_inputs_load_shared += str(
                load_op(
                    kernel_options.global_tensors_input[f"g_{k}"],
                    kernel_options.shared_tensors[f"{k}_shared"],
                    shape_idx_dim_map,
                    src_dim_list=list(range(len(shape_idx_copy_sp))),
                    src_idx_list=shape_idx_copy_sp,
                )
                + "\n"
            )
            custom_fwd_inputs_load_s2r += (
            ...
            lower_output.swizzle_shared += (
                f"{k}_shared: tl.layout.make_swizzled_layout({k}_shared), \n"
            )
        else:
...
def lower_tl(
    ...
    lower_output.is_inf_mask = (
        "True" if block_mask is not None and mask_value == "-inf" else "False"
    )
    ...
    kernel_options = AttnFwdKernelOption(
        tile_M=sp.simplify("block_M"),
        tile_N=sp.simplify("block_N"),
        dim=sp.simplify("dim"),
        dimv=sp.simplify("dimv"),
    )
    ...
    tune_output_bwd = TunnerOutputBwd(
        TUNE_BWD=str(tune_bwd), TUNE_FILE_BWD=str(tune_file_bwd)
    )
    ...
    output_idx_list = [
        i
        for i in range(
            3 + len(custom_fwd_inputs.input_tensors),
            3
    ...
    if infer_mask:
        ...
        if block_mask is not None:
            lower_output.is_casual = (
                "True"
                if is_less_causal_mask(
                    block_mask, infer_mask_block_M, infer_mask_block_N
                )
                else "False"
            )
        else:
        ...
        if (
            ...
            and not is_causal_mask(block_mask, infer_mask_block_M, infer_mask_block_N)
        ) or extern_block_mask:
