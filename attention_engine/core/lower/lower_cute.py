from ..transform.core import SymbolScalar, SymbolicArray
...
def lower_online_func(online_func, lower_cute_output: LowerCuteOutput):
    ...
    # online_fwd_epilogue
    acco = SymbolicArray(
        "acc_o_rowcol", Var("acc_o_rowcol"), shape_idx=["block_M", "dimv"]
    )
    for k, v in online_rowscales.items():
    ...
    lower_cute_output.online_fwd_body_vardefine += "\n".join(
        [
            f"Tensor {var} = make_fragment_like({list(online_rowscales.keys())[0]});"
            for var in input_vars_online
            if (var not in online_rowscales.keys() and var != "scores")
        ]
    )  # dict iter on keys
    lower_cute_output.o_scale_var = o_scalevar.varname
    lower_cute_output.copy_o_scale_var = (
        f"cute::copy({o_scalevar.varname}, scores_scale);"
    )
    lower_cute_output.FrgTensorLSE_type = "typename FrgTensorLSE, "
    for k, v in new_online_rowscales.items():
        if k != v.varname:
            lower_cute_output.copy_online_rowscales += (
                f"cute::copy({v.varname}, {k});\n"
            )

    lower_cute_output.online_rowscales_vardefine += "\n".join(
        [
            f"TensorT {var};"
            for var in new_final_rowscales.keys()
            if var not in online_rowscales.keys()
        ]
    )
    # maube  buggy because of acc_o_rowcol cannot inplace
    lower_cute_output.finalize_epilogue_body_vardefine = "\n".join(
        [
            f"Tensor {var} = make_fragment_like({list(new_final_rowscales.keys())[0]});"
            for var in input_vars_online_epilogue
            if (var not in new_final_rowscales.keys() and var != "acc_o_rowcol")
            and var not in online_rowscales.keys()
        ]
    )
    ...
    lower_cute_output.online_rowscales_0 = (
        list(online_rowscales.keys())[0] if online_rowscales else ""
    )
    lower_cute_output.online_rowscales_0_size = (
        f"size({lower_cute_output.online_rowscales_0})"
    )

    lower_cute_output.final_rowscales_params_call = ", ".join(
        [f"softmax.{varname}" for varname in new_final_rowscales.keys()]
    ) + ("," if new_final_rowscales else "")
    for k, v in new_final_rowscales.items():
        lower_cute_output.final_rowscales_struct += (
            f"float* ptr_{k};\n" + f"typename Seqlen_traits::LayoutLseT layout_{k};\n"
        )
        lower_cute_output.final_rowscales_store_code_define += (
            f"Tensor m{k} = make_tensor(make_gmem_ptr(epilogue_params.ptr_{k}), epilogue_params.layout_{k});\n"
            + f"Tensor g{k} = seqlen_traits_q.get_lse_local_tile_tensor( m{k}, Shape<Int<kBlockM>>{{}}, bidh, bidb)(_, m_block);"
        )
        lower_cute_output.final_rowscales_store_code_assert += (
            f"CUTE_STATIC_ASSERT_V(size({k}) == size(taccOcO_row));"
        )
        lower_cute_output.final_rowscales_store_code_write += f"""
    ...
    lower_cute_output.final_rowscales_store_params_def = ", ".join(
        [f"FrgTensorLSE const& {k}" for k in new_final_rowscales.keys()]
    ) + ("," if new_final_rowscales else "")
    lower_cute_output.global_ptr_args = ", ".join(
        [f"void* softmax_{k}_d" for k in new_final_rowscales.keys()]
    ) + ("," if new_final_rowscales else "")
    ...
    lower_cute_output.online_rowscale_tensor_def = "\n".join(
        [
            f"auto softmax_{k} = torch::empty({{batch_size, num_heads, seqlen_q}}, opts.dtype(at::kFloat));"
            for k in new_final_rowscales.keys()
        ]
    )
    # softmax_lse.data_ptr(),
    lower_cute_output.global_ptr_args_init = ", ".join(
        [f"softmax_{k}.data_ptr()" for k in new_final_rowscales.keys()]
    ) + ("," if new_final_rowscales else "")
    lower_cute_output.global_ptr_args_d = ", ".join(
        [f"softmax_{k}_d" for k in new_final_rowscales.keys()]
    ) + ("," if new_final_rowscales else "")
    # softmax_lse.fill_(std::numeric_limits<float>::infinity());
    # TODO: fill num
    lower_cute_output.final_rowscale_tensor_fill = "\n".join(
        [
            f"softmax_{k}.fill_(std::numeric_limits<float>::infinity());"
            for k in new_final_rowscales.keys()
        ]
    )
    # softmax_lse,
    lower_cute_output.final_rowscale_return = ", ".join(
        [f"softmax_{k}" for k in new_final_rowscales.keys()]
    ) + ("," if new_final_rowscales else "")
    # args.ptr_LSE, args.layout_LSE
    lower_cute_output.final_rowscales_return_struct = ", ".join(
        [f"args.ptr_{k}, args.layout_{k}" for k in new_final_rowscales.keys()]
    ) + ("," if new_final_rowscales else "")
    ...
    lower_cute_output.final_rowscales_store_code_write_zero = "\n".join(
        [
            f"if (thread_idx < seqlen_traits_q.actual_seq_len - m_block * kBlockM) {{ g{k}(thread_idx) = -INFINITY; }}"
            for k in new_final_rowscales.keys()
        ]
    )
    # void * __restrict__ softmax_lse_ptr;
    ...
    lower_cute_output.final_rowscales_params = ", ".join(
        [
            f"static_cast<float*>(params.softmax_{k}_ptr), seqlen_traits_q.get_lse_gmem_layout( params.seqlen_q, params.h, params.b)"
            for k in new_final_rowscales.keys()
        ]
    )  # + ("," if new_final_rowscales else "")
...
def lower_score_mod(score_mod, custom_fwd_inputs, lower_cute_output: LowerCuteOutput):
    ...
    for k, v in custom_fwd_inputs.input_tensors.items():
        if len(v.shape_idx) == 1 and v.shape_idx[0] == "1":
            ...
            lower_cute_output.score_mod_code += (
                f"const float {k} = mainloop_params.{k};\n"
            )
