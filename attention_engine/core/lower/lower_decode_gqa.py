...
TEMPLATE_PATH = osp.join(
    THIS_FILE_PATH, "../template/tl_template/attn/attn_gqa_decode_tl.py"
)
TEMPLATE_PATH_SP = osp.join(
    THIS_FILE_PATH, "../template/tl_template/attn/blockattn_decode_varlen_tl.py"
)
...
from .lower import TunnerOutput
...
def lower_tl(
    ...
    lower_output = lowerOutput(
        ...
        GROUPS=str(head),
        HEADS=str(headq),
        infer_mask_block_N=str(infer_mask_block_N),
        SEQ_LEN_KV=str(seqlenkv),
    )
    ...
    kernel_code_template = lowerKernelBaseOutput("flash_attn_split")
    ...
    output_idx_list = [
        i
        for i in range(
            4 + len(custom_fwd_inputs.input_tensors) + len(online_func.final_rowscales),
            4
            + 1
