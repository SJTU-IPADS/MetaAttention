...
TEMPLATE_PATH = osp.join(
    THIS_FILE_PATH, "../template/tl_template/attn/attn_decode_tl.py"
)
...
def lower_combine(
    ...
    for k, v in online_func.final_rowscales.items():
        final_rowscales[k] = SymbolicColReduceArray(
            v.varname, v.code, shape_idx=["num_split", "block_M2"]
        )
    combine_o_scale = combine(final_rowscales)
    tl_code, input_vars = generate_tl_from_dag(
        [combine_o_scale],
    )
    # print(tl_code)
...
def lower_tl(
    ...
    custom_fwd_inputs_list = (
        (",".join(kernel_options.global_tensors_input.keys()) + ",")
        if len(kernel_options.global_tensors_input) > 0
        else ""
    )
