...
def load_op(
    src,
    dst,
    dim_map_list: List[int],
    src_dim_list: List[int],
    src_idx_list: List[Symbol],
    src_step_list: List[Symbol] = None,
) -> str:
    """
...
def store_op(
    ...
    dst,
    dim_map_list: List[int],
    dst_dim_list: List[int],
    dst_idx_list: List[Symbol],
    dst_step_list: List[Symbol] = None,
) -> str:
...
def parallel_for_block(
    iter_size: List[str], iter_vars: List[str], body: Union[IndentedCode, str]
) -> IndentedCode:
    for_code = IndentedCode()

    for_code.add_line(
        f"for {', '.join([var for var in iter_vars])} in T.Parallel({', '.join([var for var in iter_size])}):"
    )
...
def func_block(
    func_name: str, func_args: List, func_body: Union[IndentedCode, str]
) -> IndentedCode:
