...
def bench_attention(
    ...
    if head_v is None:
        head_v = head
    common = dict(dtype=dtype, require_grad=require_grad, timer=timer)
    if attn_type == "causal_softmax_attn":
        return bench_softmaxattention(
            Batch, head, seqlen_q, seqlen_kv, dim_qk, dim_v, **common
        )
    if attn_type == "sigmoid_attn":
        return bench_sigmoidattention(Batch, head, seqlen_q, dim_qk, dim_v, **common)
    if attn_type == "gated_retention":
        return bench_gated_retention(Batch, head, seqlen_q, dim_qk, dim_v, **common)
    if attn_type == "relu_attn":
        return bench_reluattention(Batch, head, seqlen_q, dim_qk, dim_v, **common)
    if attn_type == "retention_parallel":
        return bench_retention_parallel(Batch, head, seqlen_q, dim_qk, dim_v, **common)
    if attn_type == "retention_recurrent":
        return bench_retnet_recurrent(Batch, head, seqlen_q, dim_qk, dim_v, **common)
    ...
    if attn_type == "sparse_gqa":
        return bench_sparse_gqa_decode(
            Batch,
            head,
            head_k,
            seqlen_kv,
...
def bench_softmaxattention(
    ...
    if require_grad:
        o = attention_module(query, key, value)
        ours_bwd_lat = timer(lambda: o.backward(do, retain_graph=True))
    else:
        ours_bwd_lat = None
    ...
    ours_fwd_lat_v2 = timer(ours_v2)
    if require_grad:
        o = attention_module_v2(query, key, value)
        ours_bwd_lat_v2 = timer(lambda: o.backward(do, retain_graph=True))
    else:
    ...
    try:
        ...
        fa2_fwd_lat = timer(lambda: fa2(dim_padded_fa2))
        if require_grad:
            o_ref = fa2(dim_padded_fa2)
            fa2_bwd_lat = timer(lambda: o_ref.backward(do, retain_graph=True))
        else:
            fa2_bwd_lat = None
    ...
    try:
        ...
        fa3_fwd_lat = timer(lambda: fa3(dim_padded_fa3))

        if require_grad:
            o_ref = fa3(dim_padded_fa3)
            fa3_bwd_lat = timer(lambda: o_ref.backward(do, retain_graph=True))
        else:
            fa3_bwd_lat = None
    ...
    if Sq == S:
        try:
            ...
            def flex_ref(dim_padded):
                if D < dim_padded:
                    query_padded = F.pad(query2, (0, dim_padded - D), value=0.0)
                    key_padded = F.pad(key2, (0, dim_padded - D), value=0.0)
                else:
                    query_padded = query2
                    key_padded = key2
                if DV < dim_padded:
                    value_padded = F.pad(value2, (0, dim_padded - DV), value=0.0)
                else:
                    value_padded = value2
            ...
            flex_fwd_lat = timer(lambda: flex_ref(dim_padded_flex))
            if require_grad:
                o_ref = flex_ref(dim_padded_flex)
                flex_bwd_lat = timer(lambda: o_ref.backward(do2, retain_graph=True))
            else:
                flex_bwd_lat = None
    ...
    try:
        ...
        ref_fwd_lat = timer(lambda: ref(query, key, value, causal=causal))
        if require_grad:
            o_ref = ref(query, key, value, causal=causal)
            ref_bwd_lat = timer(lambda: o_ref.backward(do, retain_graph=True))
        else:
            ref_bwd_lat = None
...
def bench_sigmoidattention(
    ...
    query = torch.randn(
        B, S, H, D, device="cuda", dtype=dtype, requires_grad=require_grad
    )
    key = torch.randn(
        B, S, H, D, device="cuda", dtype=dtype, requires_grad=require_grad
    )
    value = torch.randn(
        B, S, H, DV, device="cuda", dtype=dtype, requires_grad=require_grad
    )
    do = torch.randn(B, S, H, DV, device="cuda", dtype=dtype, requires_grad=False)

    softmax_bias = 0.1 * torch.randn(
        1, device="cuda", dtype=accum_dtype, requires_grad=False
    )
    ...
    fwd_lat = timer(lambda: attention_module(query1, key1, value1, softmax_bias_1))
    if require_grad:
    ...
    try:
        ...
        if require_grad:
            ...
            bwd_lat_ref = timer(lambda: out_ref.backward(do, retain_graph=True))
    ...
    try:
        ...
        torch_fwd_lat = timer(lambda: ref(query, key, value))
        if require_grad:
            o_ref = ref(query, key, value)
            torch_bwd_lat = timer(lambda: o_ref.backward(do, retain_graph=True))
...
def bench_reluattention(
    ...
    fwd_lat_v2 = timer(lambda: attention_module_v2(query, key, value))
    fwd_lat = min(fwd_lat, fwd_lat_v2)
    result_dict["MetaAttention"] = (fwd_lat, bwd_lat)
    ...
    ref_program_fwd_lat = timer(lambda: ref_program(query, key, value))
    if require_grad:
        out_ref = ref_program(query, key, value)
        ref_program_bwd_lat = timer(lambda: out_ref.backward(do, retain_graph=True))
    result_dict["Torch Inductor"] = (ref_program_fwd_lat, ref_program_bwd_lat)

    return result_dict


def bench_gated_retention(
    B,
    H,
    S,
    D,
    ...
    g = F.logsigmoid(torch.randn(B, H, S, device="cuda", dtype=accum_dtype)).clamp_min(
        -5
    )
    ...
    try:
        ...
        fwd_lat_ref = timer(
            lambda: chunk_simple_gla(q1, k1, v1, g1, head_first=True)[0]
        )

        if require_grad:
            out_ref, _ = chunk_simple_gla(q1, k1, v1, g1, head_first=True)
            bwd_lat_ref = timer(lambda: out_ref.backward(do, retain_graph=True))
        else:
            bwd_lat_ref = None
    ...
    try:
        ...
        fwd_lat_ref2 = timer(lambda: torch_simple_gla(q1, k1, v1, g1, chunk_size=512))

        if require_grad:
            out_ref2 = torch_simple_gla(q1, k1, v1, g1, chunk_size=512)
            bwd_lat_ref2 = timer(lambda: out_ref2.backward(do, retain_graph=True))
        else:
            bwd_lat_ref2 = None
...
def bench_retnet_recurrent(
    ...
    try:
        ...
        fwd_lat_ref = timer(lambda: chunk_retention(q1, k1, v1, head_first=True)[0])
        if require_grad:
            o_ref, _ = chunk_retention(q1, k1, v1, head_first=True)
            bwd_lat_ref = timer(lambda: o_ref.backward(do, retain_graph=True))
        result_dict["FlashLinearAttention"] = (fwd_lat_ref, bwd_lat_ref)
    except Exception as e:
    ...
    try:
        ...
        def ref(q, k, v):
            ...
            s = (
                1
                - q.new_tensor(2.0, dtype=torch.float).pow(
                    -5.0 - q.new_tensor(range(n_heads), dtype=torch.float)
                )
            ).log2()
        ...
        fwd_lat_ref2 = timer(lambda: ref(q1, k1, v1))
        if require_grad:
            o_ref2 = ref(q1, k1, v1)
            bwd_lat_ref2 = timer(lambda: o_ref2.backward(do, retain_graph=True))
        result_dict["Torch Inductor"] = (fwd_lat_ref2, bwd_lat_ref2)
    except Exception as e:
        print(f"Warning: Torch Inductor benchmark failed: {e}")
...
def bench_retention_parallel(
    ...
    torch.randn(B, S, H, DV, device="cuda", dtype=dtype, requires_grad=False)
    mask = (
    ...
    fwd_lat = timer(lambda: attention_module(q, k, v, mask))
    ...
    try:
        ...
        ref_lat = timer(lambda: ref_program(q, k, v, mask))
...
def bench_mamba2_ssm(
    ...
    if require_grad:
        do = 0.1 * torch.randn(B, S, HV, DV, dtype=dtype, device="cuda")
    ...
    try:
        ...
        def state_passing_ref(states, dA_chunk_cumsum, initial_states=None):
            ...
            causal_mask = torch.tril(
                torch.ones(nchunks, nchunks, device=states.device, dtype=bool),
                diagonal=0,
            )
        ...
        def chunk_scan_ref(B, C, x, dt, dA_cumsum, prev_states, D=None, z=None):
            ...
            causal_mask = torch.tril(
                torch.ones(chunk_size, chunk_size, device=x.device, dtype=bool),
                diagonal=0,
            )
...
def bench_mla_decode(
    ...
    fwd_lat = timer(lambda: attention_module(q, KV))
    result_dict["MetaAttention"] = (fwd_lat, None)
...
def bench_sparse_gqa_decode(
    ...
    def generate_block_mask(
        batch, heads_kv, max_cache_seqlen, sparse_ratio, cache_seqlens
    ):
        num_blocks = (max_cache_seqlen + block_size - 1) // block_size

        valid_num_blocks = torch.ceil(
            cache_seqlens * (1 - sparse_ratio) / block_size
        ).int()
        ...
        block_mask = torch.zeros(
            (batch, heads_kv, num_blocks), dtype=torch.bool, device="cuda"
        )

        # Assign valid indices while ensuring no duplicates within each batch-group
        for b in range(batch):
            max_valid_block = max_valid_num_blocks[
                b
            ].item()  # Max valid blocks for this batch
            ...
            if valid_num_block > 0:  # Ensure there's at least one valid block
                for h in range(heads_kv):
                    perm = torch.randperm(max_valid_block, device="cuda")[
                        :valid_num_block
                    ]
    ...
    # ours
    attention_module = sparse_gqa_decode(
        B, HQ, HKV, SKV, D, DV, dtype=dtype, BLOCK=block_size
    )
    fwd_lat = timer(
        lambda: attention_module(
            q, key, value, block_mask=block_mask, cache_seqlens=cache_seqlens
        ),
        warmup=100,
    )
    ...
    # triton ref
    try:
        from ref.sparse_gqa_decode_varlen_triton import (
            block_sparse_flash_decode_gqa_mask_triton,
        )

        fwd_lat_ref = timer(
            lambda: block_sparse_flash_decode_gqa_mask_triton(
                q, key, value, cache_seqlens, SKV, block_mask, block_size
            ),
            warmup=100,
        )
    ...
    try:
        sparse_mask = torch.zeros(
            B, HQ // HKV, HKV, SKV, dtype=torch.bool, device="cuda"
        )
        # Assign mask values
        for b in range(B):
            for h in range(HKV):
                for idx in range((SKV + block_size - 1) // block_size):
                    if block_mask[b, h, idx]:
                        sparse_mask[
                            b, :, h, idx * block_size : (idx + 1) * block_size
                        ] = 1
...
def run_figure11(
    result_dir: str | Path = "results",
    output_path: str | Path = "figure11_h100.pdf",
    timer: Callable[..., float] | None = None,
) -> tuple[tuple[Path, ...], Path]:
    """Run Figure 11 into explicit output paths and return created artifacts."""
    result_dir = Path(result_dir)
    output_path = Path(output_path)
    csv_paths = bench_fig11(result_dir=result_dir, timer=timer)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    from .plot_fig_h100 import plot_figure11

    plot_figure11(result_dir, output_path)
...
if __name__ == "__main__":
    print("\n" + "#" * 60)
    cprint(
        " STARTING BENCHMARK (FIGURE 11 - H100)",
        "green",
        attrs=["bold", "reverse"],
    )
    print("#" * 60 + "\n")
    start_time = time.time()
    _, output_path = run_figure11()
    ...
    print("#" * 60 + "\n")
    log_success(f"Figure 11 plotted and saved to {output_path}")
